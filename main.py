...
class RfHandler(webapp.RequestHandler):
    def get(self):
        template_values={}
        path = os.path.join(os.path.dirname(__file__), 'rf.html')
        self.response.out.write(template.render(path, template_values))

class HealthHandler(webapp.RequestHandler):
    def get(self):
        template_values={}
        path = os.path.join(os.path.dirname(__file__), 'health.html')
        self.response.out.write(template.render(path, template_values))

class EcosystemsHandler(webapp.RequestHandler):
    def get(self):
        template_values={}
...
class ScenariosHandler(webapp.RequestHandler):
    def get(self):
        template_values={}
